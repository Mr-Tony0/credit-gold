...
<head>
	<meta charset = "utf-8">
	<title>Срочные займы на карту онлайн, денежные микрозаймы срочно. Credit-gold</title>
	<meta name="description" content="Организации которые могут выдать срочный займ онлайн, деньги взаймы на карту в кротчайшие сроки"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Срочный интернет займ на банковскую карту онлайн.</h1>
			<p class="list__p">
				Бывают ситуации, когда деньги на карту нужны в короткие сроки,
				на пример не хватает до зарплаты или нужно погасить уже существующий долг.
				В таких случаях взять обычный кредит в банке не всегда возможно.
				Поэтому мы подобрали для вас список МФО,
				которые могут срочно выдать онлайн микрозайм без лишних документов и справок.
				Для получения нужной суммы потребуется паспорт и не более 5 минут вашего времени. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/srochno/"><p class="list__p active">Срочно</p></a>
				...
				<a href="/zaymy/na-kartu/bez-proverok/"><p class="list__p">Без проверок</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_srocno` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_srocno` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_srocno` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_srocno` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_srocno` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как получить срочный денежный онлайн займ на карту</h2>
			<p class="list__p">
				Микро финансовые организации могут выдать срочный онлайн займ на банковскую карту всего за несколько минут,
				пару кликов и деньги у вас на карте. С такими организациями вы сможете забыть о долгом оформлении денег, ведь,
				чтобы получить срочный займ на карту, не нужно предоставлять различные справки и доказательства вашей финансовой состоятельности.
				К тому же МФО с пониманием относятся к заемщикам и выдают новым клиентам <a href="/zaymy/na-kartu/bez-procentov/" class="active">займы на карту без процентов</a>.  
			</p>
			<h3 class="list_title third">Что бы взять срочный займ денег на карту, нужно:</h3>
			<ul class="list__ul">
				<li class="list__li">Внимательно ознакомится со списком МФО на нашем сайте</li>
				<li class="list__li">Выбрать ту организацию, которая больше всего вам подходит </li>
				<li class="list__li">Найти и осуществить нажатие по кнопке оформить</li>
				<li class="list__li">Попасть на сайт выбранной организации и оставить заявку</li>
			</ul>
			<p class="list__p">
				После этих действий вам остается только дождаться одобрения от организации,
				это займёт не более 5 минут, что позволяет получить деньги в займы на карту срочно и в режиме онлайн,
				не выходя из дома. 
			</p>
		</div>
	</section>
