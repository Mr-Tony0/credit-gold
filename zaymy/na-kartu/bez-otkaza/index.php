...
<head>
	<meta charset = "utf-8">
	<title>Список микрозаймов на карту, займы без отказа онлайн. Credit-gold</title>
	<meta name="description" content="Онлайн займы на карту без отказа, список без отказных микрозаймов"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Список онлайн займов на карту без отказа.</h1>
			<p class="list__p">
				Получить кредит в банке без отказа, может оказаться не так просто,
				ведь банки проверяют кредитную историю и место работы.
				На помощь заемщикам приходят микро финансовые организации,
				они вполне могут выдать займ на карту без отказа и лишних проверок документов. 
				Срочно нужен <a href="/zaymy/pod-zalog/pts/" class="active">займ под залог птс<a>? Оформите микрозайм онлайн на карту без отказов.
				Выберите из списка проверенных МФО на нашем сайте,
				организацию, которая предлагает самые выгодные условия. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				<h2 class="list__title">Условия</h2>
				<a href="/zaymy/na-kartu/bez-otkaza/"><p class="list__p active">Без отказа</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezOtkaza` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezOtkaza` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezOtkaza` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezOtkaza` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezOtkaza` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как получить микрозайм на карту без отказа?</h2>
			<p class="list__p">
				Для получения денег от МФО, на карту без отказов не понадобится тратить много личного времени.
				Что бы взять онлайн займ на карту достаточно просмотреть списки организаций на нашем сайте 
				и выбрать подходящее предложения, которое точно одобрит нужную сумму без отказа
			</p>
			<h3 class="list_title third">Для оформления займа на карту без отказа вам понадобится:</h3>
			<ul class="list__ul">
				<li class="list__li">Просмотреть представленные предложения у нас на сайте</li>
				<li class="list__li">Выбрать выгодное предложения от МФО</li>
				<li class="list__li">Кликнуть на кнопку оформить</li>
				<li class="list__li">Подать заявку в выборной фирме</li>
			</ul>
			<p class="list__p">
				Так же, рекомендуем подавать заявление сразу в несколько мфо из списка,
				для исключения отказов.  
			</p>
		</div>
	</section>
	<footer class="footer">
