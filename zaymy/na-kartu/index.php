...
<head>
	<meta charset = "utf-8">
	<title>Займы на карту онлайн, микрозаймы денег на карточку. Credit-gold</title>
	<meta name="description" content="Оформить онлайн займ на карту любого банка, все микрозаймы на карточку"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Деньги в займ на карту онлайн</h1>
			<p class="list__p">
				Нужный деньги в займы онлайн на карту?
				В этом разделе вы найдете интересующие вас предложения от лучших МФО страны.
				Выберете подходящее предложение и получите микрозайм на карту, не выходя из дома,
				это потребует не более 5 минут вашего времени. У нас вы найдете организации выдающие деньги на любую карту, любого банка. 
			</p>
		</div>
		<div class="list__block">
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Онлайн заявка на займ на карту</h2>
			<p class="list__p">
				Микро финансовые организации выдают деньги с помощью интернета,
				любыми удобными способами, нужно оформить <a href="/zaymy/pod-zalog/nedvizhimosti/" class="active"> займ под залог недвижимости</a> или <a href="/zaymy/na-qiwi/bez-proverok/" class="active">займ на киви кошелек без проверок</a>?
				Не проблема, выдача проходи без отказа и круглосуточно, а процентная ставка редко превышает 1%.
				При первом обращении в организацию о получении займа на карту, в большинстве случаев ставка равняется 0%. 
			</p>
			<h3 class="list_title third">Для оформления необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">Посетить раздел все займы</li>
				<li class="list__li">Нажать на пункт меню займ на карту</li>
				<li class="list__li">Определить наиболее выгодную для себя организацию из списка </li>
				<li class="list__li">Нажать кнопку оформить</li>
			</ul>
			<p class="list__p">
				Мы представляем для вас полный перечень организаций,
				специализирующихся на микрозаймах на банковскую карточку,
				работающих в режиме онлайн. Рекомендуем вам подать заявку сразу в несколько организаций,
				для более вероятного одобрения 
			</p>
		</div>
	</section>
	<footer class="footer">
