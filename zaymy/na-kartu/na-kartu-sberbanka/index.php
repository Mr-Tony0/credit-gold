...
<head>
	<meta charset = "utf-8">
	<title>Оформить микрозайм онлайн на карту сбербанка, все займы на сбербанк.</title>
	<meta name="description" content="Каталог лучших предложений для оформления онлайн займа на карточку сбербанка, получите деньги на сбербанк не тратя время на поиск подходящих предложений"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Онлайн займ на карту сбербанк.</h1>
			<p class="list__p">
				Карта сбербанк является одной из самых популярных карт в России,
				поэтому при составлении списка займов на карту,
				мы не могли проигнорировать сбербанк. В данном перечни представлены МФО,
				которые выдают микрозаймы именно на карту сбербанка,
				ведь для оформления денег в займы не каждому хочется тратить время на оформление отдельной карты.
				гораздо проще получить микрозайм онлайн на свою сбербанк карточку. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/na-kartu-sberbanka/"><p class="list__p active">На карту сбербанк</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_sberbank` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_sberbank` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_sberbank` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_sberbank` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_sberbank` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как получить онлайн займ на карту сбербанка.</h2>
			<p class="list__p">
				Если вы являетесь владельцем карты сбербанк,
				значит при оформлении займа онлайн на карту,
				вы не потратите много времени на оформлении сторонних карт.
				Многие МФО специализируются на выдаче денег именно на платежную карту сбербанка,
				поэтому для получения денег, нужно выполнить несколько простых действий,
				что не займет много времени 
			</p>
			<h3 class="list_title third">Для оформления денежного займа на карту сбербанк вам понадобится:</h3>
			<ul class="list__ul">
				<li class="list__li">Перейти к ознакомлению со списком займов у нас на сайте</li>
				<li class="list__li">Подобрать выгодное предложение </li>
				<li class="list__li">Нажать на кнопку оформить</li>
				<li class="list__li">Начать заполнять заявку на сайте МФО </li>
			</ul>
			<p class="list__p">
				Как видите, получение нужной суммы не занимает много времени,
				что позволяет быстро получить одобренный займ от микро финансовых организаций. 
			</p>
		</div>
	</section>
	<footer class="footer">
