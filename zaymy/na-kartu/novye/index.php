...
<head>
	<meta charset = "utf-8">
	<title>Новые онлайн займы на карту, самые свежие микрозаймы и мфо</title>
	<meta name="description" content="Новинки онлайн микрозаймов на карту, новые займы с новыми условиями"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Список самых новых онлайн займов на карту.</h1>
			<p class="list__p">
				Как правило новым клиентам МФО одобряют займ без процентов,
				поэтому мы подготовили подборку проверенных организаций на рынке,
				которые работают не так давно, что бы вы могли выбрать для себя новые МФК,
				которые выдают займы на карту. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/novye/"><p class="list__p active">Новые</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_novie` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_novie` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_novie` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_novie` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_novie` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как взять займ на карту в новых МФО в 2021 году.</h2>
			<p class="list__p">
				Схема получения денег от микро финансовых организаций,
				за последние несколько лет мало изменилась.
				Для получения нужной суммы, вам по-прежнему необходим только паспорт,
				телефон и доступ к интернету. Новые организации в отличии от старых ведут активную борьбу за клиентов,
				в связи с этим с каждым годом, для получения микрозайма на карту в новых организациях,
				условия займов становятся наиболее выгодными, что не может не радовать заемщиков. 
			</p>
			<h3 class="list_title third">Что бы получить займ на карту онлайн в новых организациях, необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">Просмотреть представленные на нашем сайте организации и их условия</li>
				<li class="list__li">Сделать выбор в пользу подходящей для вас организации</li>
				<li class="list__li">Нажать по кнопке оформить</li>
				<li class="list__li">На сайте организации, которую вы выбрали, подать заявку</li>
			</ul>
			<p class="list__p">
				Из представленного выше списка действий можно сделать вывод,
				что получить онлайн займ на карту в новых мфо, так же просто,
				как и в ранее вам известных организациях.
				А новые и как правило более выгодные условия,
				делают такие фирмы более привлекательными.
			</p>
		</div>
	</section>
	<footer class="footer">
