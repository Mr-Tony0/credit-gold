...
<head>
	<meta charset = "utf-8">
	<title>Деньги в займ онлайн на карту без процентов, безпроцентные микрозаймы</title>
	<meta name="description" content="Безпроцентные микрозаймы на карту, самые выгодные предложения для займа денег без процентов"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Список займов онлайн на карту без процентов.</h1>
			<p class="list__p">
				МФО, в отличии от банков, относятся к заемщикам более лояльно.
				Такие организации предлагают своим клиентам займы на карту без процентов,
				при этом не нужно долго ждать оформления и собирать различные документы для того,
				чтобы получить онлайн займ на карту без процентов достаточно при себе иметь паспорт.
				К тому же микро финансовые организации в большинстве случаев готовы выдать денежную сумму новым клиентам под 0%.
				Выдача происходит круглые сутки, у организаций нет выходных. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/bez-procentov/"><p class="list__p active">Без процентов</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezProcentov` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezProcentov` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezProcentov` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezProcentov` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bezProcentov` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как оформить займ онлайн на карту без процентов.</h2>
			<p class="list__p">
				Что бы получить нужную сумму в МФО, достаточно иметь при себе паспорт,
				такие фирмы не проверяют кредитной истории заёмщика,
				место работы и других документов,
				на сбор которых часто уходит драгоценное время. 
			</p>
			<h3 class="list_title third">Что бы получить онлайн займ на карту без процентов, необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">Выбрать нужную категорию займов у нас на сайте.</li>
				<li class="list__li">Из перечня предложений выбрать наиболее выгодное</li>
				<li class="list__li">В блоке с предложением найти и нажать кнопку оформить</li>
				<li class="list__li">Приступить к заполнению заявке на сайте фирмы.</li>
			</ul>
			<p class="list__p">
				Из всего выше сказанного можно сделать вывод,
				что получить без процентный микрозайм на карту любого банка,
				может любой человек достигший 18 лет и имеющий доступ к интернету,
				для оформления микрозайма онлайн. 
			</p>
		</div>
	</section>
	<footer class="footer">
