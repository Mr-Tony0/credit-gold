...
<head>
	<meta charset = "utf-8">
	<title>Быстрые онлайн займы на карту, получи микрозайм на карту быстро</title>
	<meta name="description" content="Подборка быстрых онлайн займов на карту, когда деньги нужны быстро, быстрые микрозаймы на карту"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Самые быстрые онлайн займы на карту.</h1>
			<p class="list__p">
				Микрозаймы в отличии от традиционного кредита,
				оформляются быстро и без лишних проверок и очередей.
				Для посетителей нашего сервиса мы подготовили список самых быстрых займов на карту.
				Мы заботимся о сохранности вашего времени, поэтому в перечни вы не найдете мфо,
				в которых выдача денежных средств происходит долго. Все организации выдают онлайн микрозаймы быстро,
				после оформления деньги поступят вам на карту уже через несколько минут. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/bystro/"><p class="list__p active">Быстро</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bystro` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bystro` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bystro` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bystro` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_bystro` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как взять быстрый займ онлайн на карту.</h2>
			<p class="list__p">
				Что бы получить быстрый микрозайм на карту в режиме онлайн,
				достаточно выполнить несколько простых действий, которые не требуют больших затрат времени. 
			</p>
			<h3 class="list_title third">Чтобы оформить быстрый онлайн займ денег на карту, необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">На нашем сайте детально изучить список самых быстрых онлайн займов на карту</li>
				<li class="list__li">Выбрать выгодное и подходящее предложение</li>
				<li class="list__li">Нажать на кнопку оформить</li>
				<li class="list__li">Заполнить заявку на сайте МФО</li>
			</ul>
			<p class="list__p">
				Как видите, что бы быстро взять микрозайм на карту,
				достаточно потратить совсем не много времени.
				К тому же МФО готовы выдавать деньги новым клиентам без процентов,
				что в свою очередь делает предложения микро финансовых организаций не только быстрыми,
				но и выгодными. Так же, чтобы исключить вероятность отказов,
				рекомендуем отправлять звяки сразу в несколько организаций,
				тем самым исключив все возможные отказы. 
			</p>
		</div>
	</section>
	<footer class="footer">
