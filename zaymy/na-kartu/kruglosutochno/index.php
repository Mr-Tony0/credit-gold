...
<head>
	<meta charset = "utf-8">
	<title>Круглосуточные займы и микрозаймы онлайн на банковскую карту</title>
	<meta name="description" content="Все организации предлогающие круглосуточные онлайн займы на карту"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Взять займ на карту онлайн, круглосуточно.</h1>
			<p class="list__p">
				Деньги на карту могут понадобится в любой момент времени,
				для таких ситуаций, когда срочно нужно получить микрозайм,
				мы составили подборку проверенных МФО, которые могут выдать нужную сумму круглосуточно.
				Перечисленные на этой странице организации работают без праздников и выходных,
				что позволяет оформить онлайн микрозайм на карту круглосуточно. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/kruglosutochno/"><p class="list__p active">Круглосуточно</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_kruglosut` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_kruglosut` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_kruglosut` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_kruglosut` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_kruglosut` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как оформить круглосуточный займ на любую карту.</h2>
			<p class="list__p">
				Получение денег от перечисленных микро финансовых организаций занимает совсем не много времени,
				к тому же все организации из списка выдают онлайн займ на карту любого банка круглосуточно.
				Так же подобные фирмы идут на уступки своим клиентам, в отличии от традиционных банков.
				Новым клиентам МФО как правило выдаются деньги без процентов. 
			</p>
			<h3 class="list_title third">Для того что бы получить круглосуточный онлайн займ на карту, вам понадобится:</h3>
			<ul class="list__ul">
				<li class="list__li">На нашем сайте, выбрать каталог займы на карту круглосуточно</li>
				<li class="list__li">Подобрать выгодное предложение исходя из суммы займа</li>
				<li class="list__li">Совершить нажатие по кнопке оформить  </li>
				<li class="list__li">Заполнить заявку на странице МФО</li>
			</ul>
			<p class="list__p">
				Как видите все вышеперечисленные действия просты и не требуют много времени на поиск нужной организации,
				на нашем ресурсе можно быстро и удобно выбрать качественное предложение. 
			</p>
		</div>
	</section>
	<footer class="footer">
