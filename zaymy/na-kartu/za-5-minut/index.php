...
<head>
	<meta charset = "utf-8">
	<title>Онлайн займ за 5 минут на карту, оформите микрозайм на карту за 5 минут</title>
	<meta name="description" content="Нужно получить одобренный онлайн займ за 5 минут? У нас вы сможете получить онлайн микрозайм за 5 минту на карту"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Онлайн микрозаймы на карту за 5 минут.</h1>
			<p class="list__p">
				Что бы оформить классический кредит, обычно нужно довольно много времени,
				но существуют организации, которые выдают деньги в займы всего за 5 минут,
				вам на карту. В этом списке перечислены МФО,
				которые выдают онлайн микрозаймы на карту в течении короткого времени, всего за 5 минут. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/za-5-minut/"><p class="list__p active">за 5 минут</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_za5Minut` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_za5Minut` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_za5Minut` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_za5Minut` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_za5Minut` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как взять микрозайм на карту за 5 минут.</h2>
			<p class="list__p">
				Большинство микро финансовых организаций не запрашивают у заемщика большого количества документов.
				Как правило, что бы оформить онлайн микрозайм за 5 минут на вашу банковскую карту,
				из документов понадобится только паспорт. 
			</p>
			<h3 class="list_title third">Что бы получить онлайн микрозайм на карту за 5 минут, нужно:</h3>
			<ul class="list__ul">
				<li class="list__li">Ознакомится с условиями МФО у нас на сайте</li>
				<li class="list__li">Найти подходящее для вам предложение </li>
				<li class="list__li">Перейти к оформлению заявки на сайте организации. </li>
				
			</ul>
			<p class="list__p">
				Достаточно совершить эти простые действия и деньги окажутся на вашей карте уже в течении 5 минут,
				оформление займа происходит очень быстро. МФО не запрашивает у своих клиентов различных справок и выписок,
				а также не проверяет кредитную историю, что позволяет получить займ без отказа.
			</p>
		</div>
	</section>
	<footer class="footer">
