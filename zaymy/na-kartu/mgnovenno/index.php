...
<head>
	<meta charset = "utf-8">
	<title>Получить займ на карту онлайн мгновенно, мгновенные микрозаймы</title>
	<meta name="description" content="Нет времени на долгое ожидание онлайн займа на карту? Тут вы сможете получить микрозайм мгновенно на любую карту."> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Список мгновенных онлайн займов на карту</h1>
			<p class="list__p">
				Для тех, у кого нет времени на долгое ожидание одобрения зйма на нужную сумму,
				мы подготовили специальную подборку, где представленный МФО с мгновенным одобрением онлайн займа на карту.
				Получить микрозайм любой суммы можно всего в несколько кликов,
				без долгих ожиданий, прямо на вашу карту, мгновенно. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-kartu/mgnovenno/"><p class="list__p active">Мгновенно</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_mgnoveno` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_mgnoveno` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_mgnoveno` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_mgnoveno` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_kartu_mgnoveno` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как оформить мгновенный онлайн займ на карту. </h2>
			<p class="list__p">
				В отличии от классических займов,
				мгновенный микрозайм совершенно не требует траты времени,
				для его получения достаточно совершить несколько кликов,
				после чего деньги поступят вам на карту в автоматическом режиме.
				Весь процесс не отличим от стандартного оформления микрозайма,
				единственное отличие в том, что, принятие вашей заявки происходит сразу,
				что позволяет получить займ денег на карту мгновенно в онлайн режиме. 
			</p>
			<h3 class="list_title third">Что бы получить мгновенный онлайн займ на карту необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">Ознакомится с представленными на сайте МФО и их предложениями</li>
				<li class="list__li">Найти интересующее вас предложение</li>
				<li class="list__li">Перейти к оформлению заявки на сайте организации</li>
			
			</ul>
			<p class="list__p">
				Так же, чтобы исключить отказ в принятие вашей заявки,
				рекомендуем подавать заявление сразу в несколько микро финансовых организаций. 
			</p>
		</div>
	</section>
	<footer class="footer">
