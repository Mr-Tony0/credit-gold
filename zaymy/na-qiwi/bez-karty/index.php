...
<head>
	<meta charset = "utf-8">
	<title>Займ на киви кошелек без карты, онлайн займы без привязки и регистрации карты</title>
	<meta name="description" content="Получить займ на киви без карты, онлайн займы на киви без банковской карты"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Взять займ без карты на киви кошелек</h1>
			<p class="list__p">
				Бывают ситуации, когда может понадобится займ на киви без привязки банковской карты или без карты вообще.
				Для таких случаев мы подготовили топ МФО,
				которые готовы выдать онлайн микрозайм на киви кошелек в короткие сроки и под выгодные проценты. 
				Все представленные организации являются проверенными и надежными, с максимально понятными условиями. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				<h2 class="list__title">Условия</h2>
				<a href="/zaymy/na-qiwi/bez-karty/"><p class="list__p active">Без карты</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bezKarti` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bezKarti` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bezKarti` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bezKarti` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
				...
				<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bezKarti` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как получить займ без карты на киви кошелек?</h2>
			<p class="list__p">
				Отсутствует банковская карта? Не проблема, представленные на странице организации готовы выдать микрозайм без регистрации карты.
				Как правило, новым клиентам микро финансовых организации доступно без процентное оформления.
				Что бы оформить онлайн займ на киви кошелек без карты вам понадобится сделать несколько простых действий:  
			</p>
			<ul class="list__ul">
				<li class="list__li">Посетить раздел <a href="/zaymy/" class="active">Все займы<a> </li>
				<li class="list__li">Выбрать подраздел займ на киви кошелек </li>
				<li class="list__li">Далее, в пункте меню займ на киви кошелек без карты выбрать нужную МФО</li>
				<li class="list__li">Нажать кнопку оформить</li>
			</ul>
			<p class="list__p">
				Для того что бы точно получить одобрения от МФО, 
				рекомендуем оформлять займ сразу в нескольких фирмах, на нашем сайте 
			</p>
		</div>
	</section>
	<footer class="footer">
