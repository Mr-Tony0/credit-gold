...
<head>
	<meta charset = "utf-8">
	<title>быстрый займ на киви кошелёк онлайн, оформить займ быстро. Credit-gold</title>
	<meta name="description" content="Нет времени на долгое оформление? Наш список быстрых займов на киви кошелек поможет вам оформить быстрый займ онлайн "> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Взять быстрый займ на киви кошелек.</h1>
			<p class="list__p">
				Займы на киви кошелек считаются одними из самых быстрых типов микрозаймов денег.
				Для оформления нужной суммы достаточно иметь при себе паспорт, подключение к интернету,
				мобильный телефон и сам киви кошелек. Оформление быстрого онлайн займа на киви кошелек не займет много времени,
				достаточно выбрать интересующее предложение из представленного на нашем сайте списка МФО и оформить анкету на сайте организации,
				все эти действия не займут у вас много времени, что позволит быстро и в короткие сроки получить займ на киви кошелек. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				...
				<a href="/zaymy/na-qiwi/bystro/"><p class="list__p active">Быстро</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bistro` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bistro` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bistro` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bistro` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `na_qiwi_bistro` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как взять быстрый займ на киви кошелек.</h2>
			<p class="list__p">
				Для оформления микрозайма вам необходимо выполнить следующие действия: 
			</p>
			<ul class="list__ul">
				<li class="list__li">Просмотреть перечень организаций, представленных на нашем сайте.</li>
				<li class="list__li">Найти подходящее МФО.</li>
			...
			<p class="list__p">
				Как видите все довольно просто и быстро. Получить деньги от микро финансовой организации можно в любое время суток,
				МФО работают без праздников и выходных, что позволяет не дожидаться открытия или проходить через длинные очереди,
				получить займ на киви кошелек можно быстро в режиме онлайн. Так же для того, чтобы исключить отказ в одобрении денег,
				мы рекомендуем оформлять микрозайм сразу в нескольких организациях. 
			</p>
		</div>
	</section>
	<footer class="footer">
