...
<head>
	<meta charset = "utf-8">
	<title>Займы под залог, оформить займ под залог. Credit-gold</title>
	<meta name="description" content="Нужны деньги? Оформи займ под залог, полный список займов и мфо."> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">займ под залог</h1>
			<p class="list__p">
				Топ МФО, которые выдают займы под залог.
				Здесь вы сможете подобрать нужную вам организацию, и воспользоваться интересующим вас предложением,
				для этого выберете из перечня организаций,
				ту которая предлагает наиболее выгодные условия для получения займов под залог.
				Выдача денег происходит мгновенно, что избавляет от томительного ожидания
			</p>
		</div>
		<div class="list__block">
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как оформить займ под залог</h2>
			<p class="list__p">
				Если вам не хочется терять время и разбираться в условиях различных организациях самостоятельно,
				воспользуйтесь нашим сервисом по подобру онлайн займа под залог птс.
				Что бы выбрать необходимую микро финансовую организацию достаточно отсортировать предложения,
				представленные на нашем сайте, по нужному критерию. Например по сумме денежных средств или по процентной ставке.
				Оформление происходит моментально, на карту или наличными
			</p>
			<h3 class="list_title third">Что бы взять займ под залог, необходимо:</h3>
			<ul class="list__ul">
				<li class="list__li">Перейти во вкладку <a href="/zaymy/" class="active">все займы</a></li>
				<li class="list__li">В меню выбрать займ под залог</li>
				<li class="list__li">Подобрать наиболее интересные предложения</li>
				<li class="list__li">Нажать кнопку оформит</li>
			</ul>
			<p class="list__p">
				Выдача микрозайма осуществляется любым удобным для клиента способом,
				на карту любого банка. Для 100% одобрения,
				рекомендуем выбрать несколько организаций и оформить займ под залог сразу в нескольких МФО.
			</p>
		</div>
	</section>
	<footer class="footer">
