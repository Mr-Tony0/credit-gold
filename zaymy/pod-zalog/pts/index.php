...
<head>
	<meta charset = "utf-8">
	<title>Займы под залог птс, оформить займ в мфо онлайн. Credit-gold</title>
	<meta name="description" content="Мы поможем подобрать подходящий займ под залог птс, оформите займ онлайн в несолько кликов"> 
...
<body>
	...
	<section class="list">
		<div class="list__text">
			<h1 class="list__title first">Займы под залог птс</h1>
			<p class="list__p">
				Все микро финансовые организации, где есть возможность получить в короткий промежуток времени займ под залог птс и с самыми низкими процентами.
				На нашем сайте есть возможность сравнить предложения от МФО и подать онлайн заявку на получение микрозайма под залог птс,
				на карту любого банка или наличными в отделении организации. 
			</p>
		</div>
		<div class="list__block">
			<aside class="list__aside">
				<h2 class="list__title">Условия</h2>
				<a href="/zaymy/pod-zalog/pts/"><p class="list__p active">Под залог птс</p></a>
			...
			<div class="list__content">
				...
				<div class="list__general">
					...
					if($_GET["sort"] == ""){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog_pts` = 'on'");
					...
					if ($_GET["sort"] == "procent"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog_pts` = 'on' ORDER BY `procent` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "sum"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog_pts` = 'on' ORDER BY `sum` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
					...
					if($_GET["sort"] == "srok"){
						$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog_pts` = 'on' ORDER BY `term_ot` ASC");
						while ($result_elements  = mysqli_fetch_array($elements)){	
							echo 
							'<div class="list__row">
								<div class="list__element">
			...
			<div class="recommendations">
					...
					<div class="recommendations__block">
						<?php
							$conect = mysqli_connect('localhost','root','','credit-gold');
							$elements = mysqli_query($conect,"SELECT `name`,`img`,`term_ot`,`term_do`, `sum`, `procent`, `link` FROM `zaymy` WHERE `pod_zalog_pts` = 'on' ORDER BY `pay` DESC LIMIT 3");
									while ($result_elements  = mysqli_fetch_array($elements)){	
										echo 
		...
		<div class="list__text">
			<h2 class="list__title second">Как взять займ под залог птс онлайн</h2>
			<p class="list__p">
				Для получения денег в МФО, вам не потребуется много времени,
				у нас вы сможете подобрать нужное предложение в короткие сроки,
				это займет не более 5 минут. Мы рекомендуем направлять заявки сразу в несколько организаций,
				для того что бы с большей вероятностью получить нужную сумму. 
			</p>
			<h3 class="list_title third">Для получения займа под залог птс вам понадобится: </h3>
			<ul class="list__ul">
				<li class="list__li">Открыть вкладку <a href="/zaymy/" class="active">все займы</a></li>
				<li class="list__li">На открывшейся странице найти займы под залог птс</li>
				<li class="list__li">Найти подходящее предложение </li>
				<li class="list__li">Нажать кнопку оформит</li>
			</ul>
			<p class="list__p">
				После этого вас направит на страницу МФО,
				где останется ввести нужную сумму и получить одобрение. 
				Мы стараемся сэкономить ваше время, что бы поиски нужного микрозайма занимали не более 5 минут,
				по этому на странице представлены только лучшие и проверенные микро финансовые организации страны. 
			</p>
		</div>
	</section>
	<footer class="footer">
